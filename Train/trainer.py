...
from trl.trainer.dpo_trainer import DPOTrainer as _TRL_DPOTrainer  # not used directly, kept for reference
...
#         comp_lens = (labels != -100).sum(dim=1).clamp_min(1)        # [2K]
#         seq_logps = seq_logps / comp_lens                           # normalize by length
#         K = ids_chosen.size(0)
#         chosen_logps = seq_logps[:K]
#         reject_logps = seq_logps[K:]
...
#         # After computing scalar grpo_loss and dpo_loss:
#         grpo = float(grpo_loss.detach())
#         dpo  = float(dpo_loss.detach())
#         r = (self.lambda_pair * dpo) / max(1e-9, grpo)
